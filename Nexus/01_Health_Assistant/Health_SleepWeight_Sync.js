...
function writeCombinedToTargetXlsx(sleepResult, weightResult, method, traceId) {
  ...
  log_(traceId, "COPY done", { tempSSId });
  ...
  try {
    ...
    log_(traceId, "SHEETS loaded", { rawLastRow: rawSheet.getLastRow(), summaryLastRow: summarySheet.getLastRow() });

    const cfg    = loadSleepConfig_(configSheet, tz, traceId);
    const colMap = getColMapFromHeader(summarySheet, 1);

    const required = [CONFIG.SUMMARY_COLS.sleepTime, CONFIG.SUMMARY_COLS.sleepStages, CONFIG.SUMMARY_COLS.sleepResult, CONFIG.SUMMARY_COLS.weight];
    ...
    let sleepMsg = "sleep=none", weightMsg = "weight=none";
    let summarySleepMsg = "summary_sleep=none", summaryWeightMsg = "summary_weight=none";
    ...
    return textResponse(okText);

  } catch (err) {
    log_(traceId, "ERR writeCombined", { error: String(err), stack: err && err.stack ? String(err.stack) : "" });
    return textResponse(`ERR_WRITE_${method} ${String(err)} TRACE=${traceId}`);
  } finally {
    try { DriveApp.getFileById(tempSSId).setTrashed(true); log_(traceId, "TEMP trashed"); }
    catch (e) { log_(traceId, "TEMP trash failed", { error: String(e) }); }
  }
}
...
function upsertSleepCalendarEvent_(rawSheet, row, sleepResult, oldEventId, cfg, tz, traceId) {
  try {
    ...
    const expectDesc  = [
      `inbed=${sleepResult.inBedMin||0}`, `core=${sleepResult.coreMin||0}`,
      `deep=${sleepResult.deepMin||0}`,   `rem=${sleepResult.remMin||0}`,
      `actual=${actual}`,                  `debt_delta=${debtDelta}`,
    ...
    if (oldEventId) {
      try {
        ...
        if (event.getTitle() === expectTitle && event.getDescription() === expectDesc) {
          log_(traceId, "CALENDAR no change, skip", { oldEventId });
          return;
        }
        event.deleteEvent();
        log_(traceId, "CALENDAR old deleted (changed)", { oldEventId });
      } catch (e) {
        log_(traceId, "CALENDAR old delete failed, recreate", { oldEventId });
      }
    }
    ...
    log_(traceId, "CALENDAR event created", { newEventId, title: expectTitle });
  } catch (err) {
